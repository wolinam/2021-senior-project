...
image_clustered = zeros(407,608);%clustered 이미지
...
new_means = zeros(k,1); % 새로 계산된 means값이 저장될 공간



% 동작 함수----------------------------------------------------------------------------------
while(true)
    convergence = (means==new_means); % convergence는 수렴 조건
    ...
    if(flag==1) % 기존 means와 new means가 완전히 동일하거나, 횟수가 50을 초과하면 중단
        ...
        for i = 1:407
            for j = 1:608
                idx = idx+1; % 각 intensity를 배열에 저장 할 때 사용할 column index
                for k_increase = 1:k % 각각의 요소와 menas 사이에 distance 함수 사용
        ...
        % 2) 새로운 means 계산 및 저장
        for k_increase = 1:k
            % 군집별로 요소들의 intensity를 평균냄. 새롭게 계산된 평균값을 new_means에 저장
